// apps/dashboard/src/components/CreateAgentModal.tsx
'use client';

import React, { useState, useRef } from 'react';
import { X, Upload, User, Plus, Bot } from 'lucide-react';
...
export const CreateAgentModal: React.FC<CreateAgentModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addAgent({
      ...
      soulMarkdown: `# ${name}\n\nI am ${title}. How can I assist you today?`,
    });
            ...
            <div className="p-10">
              <div className="flex justify-between items-start mb-10">
                <div className="flex items-center gap-5">
                   ...
                   <div>
                      <h2 className={cn("text-3xl font-extrabold tracking-tight", theme === 'dark' ? "text-white" : "text-slate-900")}>Deploy Soul</h2>
                      <p className={cn("text-sm font-medium", theme === 'dark' ? "text-slate-500" : "text-slate-400")}>Initialize a new autonomous neural identity.</p>
                   </div>
                </div>
                <button 
                  onClick={onClose}
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                   ...
                   >
                    {profilePic ? (
                      ...
                      <div className={cn(
                        ...
                        <Upload size={32} />
                        <span className="text-[10px] font-bold uppercase tracking-widest">Aura Pic</span>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className={cn("text-[10px] font-bold uppercase tracking-[0.25em] ml-1", theme === 'dark' ? "text-slate-600" : "text-slate-400")}>Identity Alias</label>
                    <input
                      type="text"
                      required
                      value={name}
                  ...
                  <div className="space-y-3">
                    <label className={cn("text-[10px] font-bold uppercase tracking-[0.25em] ml-1", theme === 'dark' ? "text-slate-600" : "text-slate-400")}>Designation</label>
                    <input
                      type="text"
                      required
                      value={title}
                ...
                <div className="pt-8 flex gap-5">
                  ...
                  <button
                    type="submit"
                    className="flex-[2] px-8 py-4 rounded-[20px] bg-indigo-600 text-white font-bold uppercase tracking-widest text-[11px] shadow-xl shadow-indigo-600/30 hover:bg-indigo-500 transition-all active:translate-y-1"
                  >
