// apps/dashboard/src/components/SlideToConfirm.tsx
'use client';
...
export const SlideToConfirm: React.FC<SlideToConfirmProps> = ({ onConfirm, label }) => {
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative h-14 rounded-2xl border transition-all duration-300 flex items-center px-2 ${
        isConfirmed 
    ...
    >
      ...
      <div
        ...
        className={`z-10 w-10 h-10 rounded-xl flex items-center justify-center cursor-grab active:cursor-grabbing transition-colors shadow-lg ${
          isConfirmed ? 'bg-white text-emerald-600' : 'bg-rose-600 text-white'
        }`}
      >
        {isConfirmed ? <Check size={20} fontWeight="bold" /> : <ChevronRight size={20} />}
      </div>
