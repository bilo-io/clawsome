...
export interface SmartHistorySearchProps {
  items: HistoryItem[];
  search: string;
    ...
    <div className="space-y-6">
      <div className="relative group">
         ...
         <input 
           ...
           className={cn(
             "w-full rounded-[24px] py-6 pl-14 pr-24 text-sm font-black tracking-tight focus:outline-none focus:ring-4 transition-all shadow-xl",
             theme === 'dark' 
         ...
         <div className={cn("divide-y", theme === 'dark' ? "divide-slate-900" : "divide-slate-50")}>
            {items.map((item, i) => (
              <motion.div 
                ...
                className={cn(
                  "p-5 transition-all flex items-center justify-between group cursor-pointer",
              ...
              >
                 <div className="flex items-center gap-6 min-w-0">
                    ...
                    <div className="min-w-0 flex flex-col gap-1">
                       <code className={cn(
                         "text-sm font-mono transition-colors truncate block selection:bg-indigo-500/30 font-bold",
                         theme === 'dark' ? "text-slate-300 group-hover:text-white" : "text-slate-700 group-hover:text-slate-950"
                       )}>
                          {item.command}
                       </code>
                       <div className="flex items-center gap-3">
                          ...
                          <span className={cn("text-[9px] uppercase font-black tracking-widest opacity-40 tabular-nums", theme === 'light' && "text-slate-500")}>
                             {item.date}
                          </span>
                       </div>
                    </div>
