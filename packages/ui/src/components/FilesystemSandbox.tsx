...
import { Folder, File, ChevronRight, Eye, ShieldAlert, Lock } from 'lucide-react';
...
export interface FilesystemSandboxProps {
  tree: SandboxItem[];
  mountedCount: number;
  totalCount: number;
  onItemClick?: (item: SandboxItem) => void;
  onViewClick?: (item: SandboxItem) => void;
}
    ...
    <div className={cn(
      ...
      <div className={cn(
        ...
        theme === 'dark' ? "border-slate-800/50" : "border-slate-50"
         ...
         <span className="text-[10px] font-mono text-emerald-500 font-black px-3 py-1 bg-emerald-500/10 rounded-lg uppercase">{mountedCount}/{totalCount} MOUNTED</span>
      </div>
        ...
        {tree.map((item, i) => (
          <motion.div 
            ...
            className={cn(
              ...
              item.mounted 
                ? (theme === 'dark' ? 'hover:bg-slate-800/50 border border-transparent hover:border-indigo-500/20 cursor-pointer' : 'hover:bg-slate-50 border border-transparent hover:border-indigo-100 cursor-pointer')
          ...
          >
            <div className="flex items-center gap-4">
               ...
               <div className="flex flex-col">
                  ...
                  {item.type === 'folder' && (
                    <span className={cn("text-[9px] font-black uppercase tracking-widest", theme === 'dark' ? "text-slate-600" : "text-slate-400")}>{item.children} assets</span>
                  )}
               </div>
            </div>
            <div className="flex items-center gap-4">
      ...
      <div className={cn(
        ...
        theme === 'dark' ? "bg-indigo-500/5 border-indigo-500/10 text-indigo-400" : "bg-indigo-50/50 border-indigo-100 text-indigo-600"
         ...
         <span className="text-[10px] font-black uppercase tracking-[0.1em] leading-relaxed">
